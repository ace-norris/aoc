...
7214296 k
	...
	for i, c := range cs {
		...
		flatten(process(c.Stream), directories)
		total := 0
		...
		// assert
		if total != c.Expected {
			t.Errorf("Case: %v, Expected: %v, Actual: %v", i, c.Expected, total)
		}
	}
}
...
7214296 k
	...
	for i, c := range cs {
		...
		size := filtered[0].CalculateSize()
		// assert
		if size != c.Expected {
			t.Errorf("Case: %v, Expected: %v, Actual: %v", i, c.Expected, size)
		}
	}
}
